    ...
    <div className="bg-gradient-to-tr from-cyan-100 via-sky-100 to-pink-200 ">
      ...
      <div className="grid grid-cols-3 p-5 mx-auto gap-3">
        {galleryData.map((item,index)=>(
          <div key={index} className="w-44 h-40 flex flex-col justify-center items-center" > 
          <Image width={1440} height={1400} src={item.img} alt={item.title} className= "w-full h-full object-cover"/>
          <span className="text-yellow-500 text-xl">{item.title}</span>
          </div>
        ))}
      </div>
      ...
      <div className="w-full md:p-10 lg:p-14 mx-auto mt-3 xl:mt-20 flex flex-wrap gap-4 lg:gap-10 2xl:gap-16 place-content-center pb-6">
        {galleryData.map((item,index)=> (
        <Link href={item.slug} key={index}><div  className="flex place-content-center items-center  w-36 h-24 sm:w-[160px] sm:h-[120px] lg:w-72 lg:h-60 md:w-[250px] md:h-[200px] xl:w-96 xl:h-80 2xl:w-[400px] 2xl:h-[350px] transition duration-300 hover:scale-105 bg-gray-500">
          <Image width={1440} height={1400} alt="rangoli-competition"
            src={item.img}
            className=" w-full h-full object-cover "
          />
          <h1 className=" text-white text-base xl:text-2xl">
            {item.title}
          </h1>
        </div>
        </Link>))}
